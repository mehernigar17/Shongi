...
///
/// The routine catalog is static content, but the *active* routine and the
/// personalized skin type come from the signed-in user's own profile
/// (`users/{uid}`), so each user sees their own state.
class FirestoreSkincareRepository implements SkincareRepository {
  ...
  Future<void> saveActiveRoutine(String routineId) async {
    await _settings.updateActiveRoutine('skincare', routineId);
  }
